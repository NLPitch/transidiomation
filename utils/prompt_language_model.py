...
def prompt_openai(input_sentence:str, target_language:str, api_key:str) -> str:
    client = OpenAI(
        api_key=api_key
    )
    ...
    # ]
    #   messages=[
    #       {"role": "system", "content": '''You are evaluating the quality of translation sentences.'''},
    #       {"role": "user", "content": f'''Given the reference text, 'I hardly go on a date because I have high standards.', rank the following sentences based on its similarity to the reference text.
          
    #           Text 1: I have high standards so I don't go on dates often.
    #           Text 2: I don't date often because I have high standards.
    #           Text 3: I don't date much because I have high standards.
    #       '''},
    #       {"role": "assistant", "content": "3; 2; 1"},
    #       {"role": "user", "content": f'''Given the reference text, {reference_text}, rank the following sentences based on its similarity to the reference text.
